...
<<<<<<< HEAD
   ...
   - [How can I test the refresh token logout workflow?](#how-can-i-test-the-refresh-token-logout-workflow)
   - [What is the uniqueness and rotation policy for EUID tokens?](#what-is-the-uniqueness-and-rotation-policy-for-euid-tokens)
   - [Should /token/generate return the “optout” status and generate no tokens if I pass optout@email.com in the request payload?](#should-tokengenerate-return-the-optout-status-and-generate-no-tokens-if-i-pass-optoutemailcom-in-the-request-payload)
...
- When you generate the EUID token by a call to the  [POST /token/generate](../endpoints/post-token-generate.md) endpoint, either directly or via one of the EUID SDKs, using the optional `policy` parameter with a value of `1`.
