[EUID API Documentation](../../../README.md) > [Getting Started](../../getting-started.md) > [v2](../summary-doc-v2.md) > [Integration Guides](README.md) > AWS Marketplace
...
- [Upgrading EUID Operator](#upgrading-euid-operator)
- [Technical Support](#technical-support)

## EUID Operator on AWS Marketplace Product

>NOTE: [EUID Operator on AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-wdbccsarov5la) is a free product. The cost displayed on the product page is an infrastructure cost estimate.

By subscribing to the EUID Operator on AWS Marketplace product, you gain access to the following:

- **Amazon Machine Image (AMI)** with the EUID Operator service installed and ready to bootstrap:<br/>
    The AMI contains a AmazonLinux2 OS with EUID Operator service set up. When an EC2 instance based on the AMI boots up, it automatically fetches configuration from your AWS account and starts the EUID Operator server inside an enclave.  
- **CloudFormation template** to deploy EUID Operator AMIs.
...
To deploy the EUID Operator on AWS Marketplace, complete the following steps:

1. Subscribe to [EUID Operator on AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-wdbccsarov5la). It might take several minutes before AWS completes your subscription.
...
It takes several minutes for the stack to be created. When you see an Auto Scaling Group (ASG) created, you can select it and check the EC2 instances (by default, there is only one instance to start with).  For details, see [Checking EUID Operator Status](#checking-euid-operator-status).
...
|OPERATOR_KEY |This is the Operator Key that you received from the EUID Admin team. |
|EUID Environment |Select `prod` for production environment or `integ` for integration test environment. |
...
4. Enter the EUID **Load balancer name** and, depending on whether you need to access EUID APIs from public internet, choose the **Internet-facing** or **Internal** scheme.
5. Select the **VPC** for your targets and at least two subnets used in your CloudFormation stack.
6. Click **Create new security group** and enter `EUIDSGALB` as its name.
...
10. Go back to the Load Balancer page, select `EUIDALBTG` under **Forward to** and change **Port** to `443`.
...
1. On the Specify group details page, select **Instances** as target type, enter `EUIDALBTG` as **Target group name**, and select **HTTP1** as the **Protocol version**.
...
4. Select EUID Operator EC2 Instances created by your auto scaling group and click **Include as pending below**. 
5. Make sure the **Ports for the selected instances** contains `80`.
6. Click **Create target group**.

## Upgrading EUID Operator
...
- Information on the availability of new versions is provided at [EUID Operator on AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-wdbccsarov5la).
