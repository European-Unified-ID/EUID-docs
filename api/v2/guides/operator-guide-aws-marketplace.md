[EUID Overview](../../../README.md) > [Getting Started: Summary](../getting-started/gs-summary.md) > [EUID API Documentation](../summary-doc-v2.md) > [EUID Integration Guides: Summary](summary-guides.md) > EUID Private Operator for AWS Integration Guide
...
| ----------- | --------- | -------- | ------ |
| 80 | Inbound | HTTP | Serves all EUID APIs, including the healthcheck endpoint `/opt/healthcheck`.<br/>When everything is up and running, the endpoint returns HTTP 200 with a response body of `OK`. For details, see [Checking EUID Operator Status](#checking-euid-operator-status).|
...
4. Enter the EUID **Load balancer name** and, depending on whether or not you need to access EUID APIs from public internet, choose the **Internet-facing** or **Internal** scheme.
...
10. Go back to the Load Balancer page. Under **Forward to**, select `EUIDALBTG`, and then change the **Port** value to `443`.
...
1. On the Specify group details page, select **Instances** as the target type, then enter `EUIDALBTG` as the **Target group name** and select **HTTP1** as the **Protocol version**.
