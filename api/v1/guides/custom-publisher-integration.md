[EUID API Documentation](../../README.md) > [v1](../README.md) > [Integration Guides](README.md) > Custom Publisher Integration Guide 

# Server-Only EUID Integration Guide
...
>TIP: To facilitate the process of establishing client identity using EUID and retrieving advertising tokens, consider using the [Client-Side Identity JavaScript SDK](../sdks/client-side-identity.md). For details, see [EUID SDK Integration Guide](./publisher-client-side.md).
...
You can use the `optout@email.com` email address to test your token refresh workflow. Using this email address in requests always generates an identity response with a `refresh_token` that results in a logout response.
