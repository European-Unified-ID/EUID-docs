...
A response state of `Expired` means that the EUID token has expired but the refresh token has not expired, therefore the EUID token can be refreshed.
...
A response state of `RefreshExpired` means that the EUID token and the refresh token have both expired, therefore the EUID token cannot be refreshed.
