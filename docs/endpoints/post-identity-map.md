...
A successful decrypted response returns the current raw EUIDs, previous raw EUIDs, and refresh timestamps for the specified email addresses, phone numbers, or their respective hashes. 
...
{
    "email": [
        "user@example.com",      // Corresponding EUID rotated in the last 90 days
        "user2@example.com",     // Corresponding EUID rotated more than 90 days ago
        "invalid email string",  // Invalid identifier
        "optout@example.com"     // Personal data is opted out
    ]
}
```
...
|:---------------|:----------------------------|:------------------------------------------------------------------------------------------------|
| `email`        | array of mapped personal data objects | The list of mapped personal data objects corresponding to the list of emails in the request.              |
| `email_hash`   | array of mapped personal data objects  | The list of mapped personal data objects corresponding to the list of email hashes in the request.        |
| `phone`        | array of mapped personal data objects | The list of mapped personal data objects corresponding to the list of phone numbers in the request.       |
| `phone_hash`   | array of mapped personal data objects | The list of mapped personal data objects corresponding to the list of phone number hashes in the request. |


For successfully mapped personal data, the mapped object includes the properties shown in the following table.
...
| `r`      | number     | The Unix timestamp (in seconds) that indicates when the raw EUID might be refreshed. The raw EUID is guaranteed to be valid until this timestamp. |
...
| Property | Data Type | Description                                                                                                      |
|:---------|:----------|:-----------------------------------------------------------------------------------------------------------------|
| `e`      | string    | The reason for being unable to map the personal data to a raw EUID. One of two possible values:<ul><li>`optout`</li><li>`invalid identifier`</li></ul> |
...
| Previous EUID Access       | Not available                               | Available for 90 days        |
...
Update your code for salt bucket monitoring, replacing it with code that checks the `refresh_from` timestamp to determine raw EUIDs that are due for refresh.
