...
| ------- | ------ | ------ | ------ | 
| Q4 2025 | [v5.62.24-r2](https://github.com/IABTechLab/uid2-operator/releases/tag/v5.62.24-r2) | 5.62.24-r2 | January 15, 2026 |
...
| :--- | :--- | :--- | :--- |
| E12 | Data Download Failure | Look for log messages containing `"E12: Data Download Failure"` along with `"Failed to load"` errors from `RotatingStoreVerticle`. These will include HTTP status codes (e.g., `"HTTP response code 403"`) or exception types (e.g., `"exception: IOException"`). | Check the HTTP status code or exception in the error message: <br/>**404 errors** - Verify operator key is valid for the environment.<br/>**403 errors** - Verify operator key and credentials are correct.<br/>**Timeout errors** - Verify network connectivity, check firewall/security group settings allow outbound HTTPS (port 443), and ensure EUID service endpoints are accessible.<br/>**500/503 errors** - Temporary EUID service issue, retry or contact EUID support if persistent. |
